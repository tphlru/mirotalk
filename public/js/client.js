...
function setButtonsToolTip() {
    ...
    // Init buttons
    setTippy(initScreenShareBtn, 'Переключить демонстрацию экрана', 'top');
    // Main buttons
    refreshMainButtonsToolTipPlacement();
    // Chat room buttons
    setTippy(msgerClose, 'Закрыть', 'bottom');
    setTippy(msgerShowChatOnMsgDiv, 'Показывать чат при получении нового сообщения', 'bottom');
    setTippy(msgerSpeechMsgDiv, 'Произносить новые сообщения вслух', 'bottom');
    setTippy(msgerTheme, 'Тема-призрак', 'bottom');
    setTippy(msgerClean, 'Очистить историю сообщений', 'bottom');
    setTippy(msgerSaveBtn, 'Сохранить сообщения', 'bottom');
    setTippy(msgerMaxBtn, 'Раскрыть', 'bottom');
    ...
    setTippy(msgerShareFileBtn, 'Отправить файо', 'top');
    ...
    setTippy(captionMaxBtn, 'Раскрыть', 'bottom');
    setTippy(captionMinBtn, 'Свернуть', 'bottom');
    setTippy(captionTheme, 'Тема-призрак', 'bottom');
    setTippy(captionClean, 'Очистить историю сообщений', 'bottom');
    setTippy(captionSaveBtn, 'Сохранить сообщения', 'bottom');
    setTippy(speechRecognitionIcon, 'Статус', 'bottom');
    setTippy(speechRecognitionStart, 'Начать', 'top');
    setTippy(speechRecognitionStop, 'Остановить', 'top');
    // Settings
    setTippy(mySettingsCloseBtn, 'Закрыть', 'bottom');
    setTippy(myPeerNameSetBtn, 'Изменить имя', 'top');
    setTippy(myRoomId, 'Название комнаты (click to copy/share)', 'right');
    setTippy(
        switchPushToTalk,
        'If Active, When SpaceBar keydown the microphone will be activated, on keyup will be deactivated, like a walkie-talkie',
        'right',
    );
    setTippy(switchSounds, 'Переключить звук уведомлений', 'right');
    setTippy(switchShare, "Показывать 'Share Room' всплывающее уведомление.", 'right');
    setTippy(recImage, 'Переключить запись', 'right');
    setTippy(
        switchH264Recording,
        'Предпочитать h.264 с AAC or h.264 с Opus кодеками или VP9 с Opus кодеками',
        'right',
    );
    // Whiteboard buttons
    ...
    setTippy(whiteboardImgUrlBtn, 'Добавить изображение по ссылке', 'bottom');
    setTippy(whiteboardTextBtn, 'Добавить текстовое поле', 'bottom');
    setTippy(whiteboardLineBtn, 'Добавить строку', 'bottom');
    setTippy(whiteboardRectBtn, 'Добавить квадрат', 'bottom');
    setTippy(whiteboardTriangleBtn, 'Добавить треугольник', 'bottom');
    setTippy(whiteboardCircleBtn, 'Добавит круг', 'bottom');
    setTippy(whiteboardSaveBtn, 'Сохранить доску', 'bottom');
    setTippy(whiteboardEraserBtn, 'Очистить объект', 'bottom');
    setTippy(whiteboardCleanBtn, 'Очистить доску', 'bottom');
    setTippy(whiteboardLockBtn, 'Заблокировать доску для участников', 'right');
    setTippy(whiteboardCloseBtn, 'Закрыть', 'right');
    // Suspend/Hide File transfer buttons
    setTippy(sendAbortBtn, 'Отменить передачу файлов', 'right-start');
    setTippy(receiveHideBtn, 'Скрыть передачу файлов', 'right-start');
    // Video/audio URL player
    setTippy(videoUrlCloseBtn, 'Закрыть видео плеер', 'bottom');
    setTippy(videoAudioCloseBtn, 'Закрыть видео плеер', 'bottom');
    setTippy(msgerVideoUrlBtn, 'Поделиться видео или аудио с другими участниками', 'top');
}
...
function refreshMainButtonsToolTipPlacement() {
    ...
    setTippy(hideMeBtn, 'Показать / Скрыть себя', placement);
    setTippy(audioBtn, useAudio ? 'Отключить мой микрофон' : 'Микрофон отключён', placement);
    setTippy(videoBtn, useVideo ? 'Остановить моё видео' : 'Видеокамера отключена', placement);
    setTippy(screenShareBtn, 'Демонстрация экрана', placement);
    setTippy(recordStreamBtn, 'Начать запись конференции', placement);
    ...
    setTippy(roomEmojiPickerBtn, 'Поставить реакцию', placement);
    setTippy(myHandBtn, 'Поднять руку', placement);
    setTippy(whiteboardBtn, 'Открыть интерактивную доску', placement);
    setTippy(fileShareBtn, 'Отправить файл', placement);
    setTippy(documentPiPBtn, 'Режим картинка в картинке', placement);
    setTippy(mySettingsBtn, 'Настройки', placement);
...
function roomIsBusy() {
    ...
    Swal.fire({
        ...
        html: `Количество участников в комнате ограничено до ${userLimits.count} пользователей. <br/> Пожалуйста, попробуйте позже.`,
...
async function whoAreYou() {
    ...
    Swal.fire({
        ...
        allowEscapeKey: false,
        background: swBg,
        title: 'RepinaRoom P2P videocall',
        position: 'center',
        input: 'text',
        inputPlaceholder: 'Введите ваше имя (псевдоним)',
        inputAttributes: { maxlength: 32 },
        inputValue: window.localStorage.peer_name ? window.localStorage.peer_name : '',
        html: initUser, // inject html
        confirmButtonText: `Подключиться к встрече`,
...
function userNameAlreadyInRoom() {
    ...
    Swal.fire({
        ...
        position: 'center',
        title: 'Username',
        html: `Это имя пользователя уже занят <br/> Пожалуйста, попробуйте другое`,
        showDenyButton: false,
        confirmButtonText: `OK`,
        showClass: { popup: 'animate__animated animate__fadeInDown' },
        hideClass: { popup: 'animate__animated animate__fadeOutUp' },
    ...
    switch (kind) {
        case 'video':
            ...
            if (!isMobileDevice) {
                ...
                setTippy(myPeerName, 'Моё имя', 'bottom');
                setTippy(myHandStatusIcon, 'Моя рука поднята', 'bottom');
                setTippy(myPrivacyBtn, 'Переключить доступность видео', 'bottom');
                setTippy(myVideoStatusIcon, 'Моё видео включено', 'bottom');
                setTippy(myAudioStatusIcon, 'Микрофон включен', 'bottom');
                setTippy(myVideoToImgBtn, 'Сделать снепшот', 'bottom');
                setTippy(myVideoFullScreenBtn, 'Полноэкранный режим', 'bottom');
                setTippy(myVideoZoomInBtn, 'Зум в видео', 'bottom');
                setTippy(myVideoPiPBtn, 'Переключить режим картинка в картинке', 'bottom');
                setTippy(myVideoZoomOutBtn, 'Zoom out video', 'bottom');
                setTippy(myVideoPinBtn, 'Закрепить', 'bottom');
            }
...
function checkShareScreen() {
    if (!isMobileDevice && isScreenEnabled && isScreenSharingSupported) {
        ...
        Swal.fire({
            ...
            position: 'center',
            icon: 'question',
            text: 'Вы действительно хотите продемонстрировать ваш экран?',
            showDenyButton: true,
    ...
    switch (kind) {
        case 'video':
            ...
            if (!isMobileDevice) {
                ...
                setTippy(remoteHandStatusIcon, 'Рука участника поднята', 'bottom');
                setTippy(remoteVideoStatusIcon, 'Видео участника включено', 'bottom');
                setTippy(remoteAudioStatusIcon, 'Микрофон участника включен', 'bottom');
                ...
                setTippy(remoteVideoToImgBtn, 'Сделать снепшот', 'bottom');
                setTippy(remotePeerKickOut, 'Выкинуть из конференции', 'bottom');
                ...
                setTippy(remoteVideoPiPBtn, 'Переключить режик картинка в картинке', 'bottom');
                setTippy(remoteVideoPinBtn, 'Закрепить', 'bottom');
            }
...
function shareRoomMeetingURL(checkScreen = false) {
    ...
    Swal.fire({
        ...
        <br/>
        <p style="color:rgb(8, 189, 89);">Используйте со смартфоном!</p>
        <p style="background:transparent; color:white; font-family: Arial, Helvetica, sans-serif;">Не нужно ничего устанавливать - просто отсканируйте QR-код или поделитесь ссылкой.</p>
...
async function sendChatMessage() {
    if (!thereArePeerConnections() && !isChatGPTOn) {
        ...
        return userLog('info', "Некому отправлять сообщение, нет других участников!");
    }
...
function handleDataChannelChat(dataMessage) {
    ...
    // show message from
    if (!showChatOnMessage) {
        userLog('toast', `Новое соообщение от: ${msgFrom}`);
    }
...
function hideShowMySettings() {
    ...
    setTippy(mySettingsBtn, 'Открыть настройик', placement);
